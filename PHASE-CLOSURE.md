...
5. **Data caveat (from 9.4)** — net-worth liabilities show 0. **Root cause confirmed 2026-07-09:** the legacy v4 sheet's Net Worth tab had *every* liability row (Car Loan, Credit Card, Other Liabilities) at 0 for all 12 months — the source never tracked them, so there is nothing to backfill; the P8 import was faithful. Fix is **manual entry only**: enter current outstanding balances for the 3 active Liability accounts via the dashboard Balance Editor. (Asset balances are also stale — latest recorded month is April 2026 — so refresh those in the same pass. BNPL/Atome outstanding is already folded in separately and needs no entry.)
