...
import { accounts, categories, netWorthEntries, transactions } from "@/db/schema";
...
export async function getAnalyticsBundle(
  ...
  const [cashflowRows, breakdownRows, nweRows, acctRows] = await Promise.all([
  ...
  return {
    ...
    netWorthTrend: netWorthTrend(nweRows, acctRows),
  };
}
...
function netWorthTrend(
  ...
  for (const month of months) {
    ...
    out.push({ month, assetsCents: assets, liabilitiesCents: liab, netCents: assets - liab });
  }
  return out;
}
